...
function classifyAffineFixedSet(linearPart, translation) {
  const system = [
  ...
  if (isZeroMatrix(system)) {
    return isZeroVector(rhs)
      ? { kind: 'plane', label: 'Todo R² fijo' }
      : { kind: 'none', label: 'Sin puntos fijos' }
  }

  const anchor = solvePossiblySingular(system, rhs)

  if (!anchor) {
    return { kind: 'none', label: 'Sin puntos fijos' }
  }

  return {
    kind: 'line',
    anchor,
    direction: kernelVectorOfSingular(system),
  ...
  if (fixedSet.kind !== 'none') {
    return {
      ...
      details: [
        ...
        'El sistema (I-A)x=b es compatible, así que se escoge una solución como nuevo origen.',
        'Tras mover el origen a ese punto fijo elegido, la clasificación afín se reduce a la lineal.',
      ],
    }
  }
...
function identityMinusMatrix(matrix) {
  return [
...
function expressionHomogeneousLinearToLatex(matrix) {
...
function homogeneousChangeOfReference(origin, basis) {
  return [
    [1, 0, 0],
...
function inverseHomogeneousChangeOfReference(origin, basis) {
...
function affineHomogeneousVerification(linearPart, translation, origin, basis) {
  ...
  if (!inverseChange) {
    return { change, transformed: null }
  }

  const source = homogeneousFromAffine(linearPart, translation)
  return {
    change,
    transformed: multiplyMatrices3(multiplyMatrices3(inverseChange, source), change),
  }
}
...
`
  ...
  if (!transformed) {
    return `
La matriz del cambio de referencia es
\\[
C=${matrixToLatex3x3(change)}.
\\]
`
  }

  return `
La matriz del cambio de referencia $x=O+Pz$ es
\\[
C=${matrixToLatex3x3(change)}.
\\]
Por la multiplicatividad de las matrices homogéneas,
\\[
C^{-1}H_FC=${matrixToLatex3x3(transformed)}=H_{\\mathrm{can}}.
\\]
`
}
...
\\]
`
    return { v1, v2, P, canonical: analysis.canonicalMatrix, pTex, canonicalTex, homogeneousTex: expressionHomogeneousLinearToLatex(canonicalEntries), hasSymbolicEntries: true, body }
  }
...
`,
  ...
  if (analysis.caseId === 'jordan-block') {
    ...
    const P = matrixFromColumns(generalized, eigen)
    return {
      v1: generalized,
      v2: eigen,
      P,
...
\\]
Ordenando la base como $(w,v)$, formamos $P=[\\,w\\ v\\,]=${matrixToLatex(P)}$, y obtenemos
\\[
J=${matrixToLatex(analysis.canonicalMatrix)}.
\\]
`,
...
function fixedSetCaseTitle(fixedSet) {
...
function fixedSetChosenOrigin(fixedSet) {
...
function buildAffineCaseWithFixedSet(linearPart, translation, fixedSet, analysis) {
...
\\]
${affineHomogeneousVerificationLatex([[1, 0], [0, 1]], translation, { x: 0, y: 0 }, P)}
...
function buildAffineCaseDistinctOneIsEigenvalue(linearPart, translation, analysis) {
  ...
  const scaledP = matrixFromColumns(scaledV1, v2)
...
function buildAffineCaseParabolic(linearPart, translation, analysis) {
  ...
  const scaledP = matrixFromColumns(scaledGeneralized, scaledEigen)
  const canonical = [[1, 0], [1, 1]]
  void analysis
  return `
\\subsection*{Paso 2. Resolver la ecuación de puntos fijos}
...
function buildAffineCaseFallback(linearPart, translation, analysis) {
  ...
  return `
\\subsection*{Paso 2. Resolver la ecuación de puntos fijos}
Este caso no corresponde a una de las tres familias sin puntos fijos que aparecen en el plano. La salida conserva la reducción lineal como referencia de comparación.

\\subsection*{Paso 3. Separar la traslación residual}
En este caso el cambio lineal de coordenadas que normaliza $A$ absorbe también la traslación sin dejar residuos esenciales.

\\subsection*{Paso 4. Reducir la parte lineal canónica}
${canonical.body}
...
\\paragraph{Comprobación de independencia afín.}
${buildAreaCalculationLatex(payload.source.p0, payload.source.p1, payload.source.p2, area)}

\\paragraph{Vectores asociados a los puntos origen y a sus imágenes.}
\\[
